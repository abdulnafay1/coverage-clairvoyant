      ...
      <nav className="flex-1 px-3 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`h-4.5 w-4.5 ${active ? "text-sidebar-primary" : ""}`} />
              {item.label}
            </button>
          );
        })}
      </nav>
      <div className="p-4 mx-3 mb-4 rounded-xl bg-sidebar-accent/50 text-xs text-sidebar-foreground/60">
